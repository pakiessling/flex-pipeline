...
parser <- arg_parser(
  "SoupX ambient RNA correction + scDblFinder doublet detection"
)
...
parser <- add_argument(
  parser,
  "--filtered_matrix",
  help = "Filtered 10x H5 matrix path"
)
parser <- add_argument(
  parser,
  "--raw_matrix",
  help = "Raw 10x H5 matrix path",
  default = NA
)
parser <- add_argument(
  parser,
  "--skip",
  help = "Skip SoupX correction",
  flag = TRUE
)
parser <- add_argument(
  parser,
  "--expected_doublet_rate",
  help = "Expected doublet rate (dbr for scDblFinder)",
  default = NA
)
...
if (args$skip) {
  ...
  result <- tryCatch(
    {
      cat(sprintf(
        "[%s] Building SoupChannel and running autoEstCont …\n",
        args$sample
      ))
    ...
    error = function(e) {
      cat(sprintf(
        "[%s] SoupX failed (%s); falling back to filtered matrix.\n",
        args$sample,
        conditionMessage(e)
      ))
  ...
  if (soupx_run) {
    ...
    cat(sprintf(
      "[%s] SoupX complete. Counts before=%.0f, after=%.0f (%+.2f%%)\n",
      args$sample,
      before_sum,
      after_sum,
      pct
...
dbr_arg <- if (!is.na(args$expected_doublet_rate)) {
  as.numeric(args$expected_doublet_rate)
} else {
  NULL
}
...
dbl_df <- tryCatch(
  ...
  error = function(e) {
    cat(sprintf(
      "[%s] scDblFinder failed (%s); filling with NA.\n",
      args$sample,
      conditionMessage(e)
    ))
    data.frame(
...
var_df <- data.frame(row.names = rownames(filt_mat), stringsAsFactors = FALSE)
  ...
  uns = list(
    pipeline_log = list(
      ...
      scdblfinder = list(
        ...
        expected_doublet_rate = if (!is.na(args$expected_doublet_rate)) {
          as.numeric(args$expected_doublet_rate)
        } else {
          NULL
        },
