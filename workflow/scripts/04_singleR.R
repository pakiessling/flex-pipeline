...
parser <- add_argument(
  parser,
  "--plot_dir",
  help = "Directory to save UMAP plots (default: same dir as output)",
  default = ""
)
...
# --- UMAP plots ------------------------------------------------------------
plot_dir <- if (nchar(trimws(args$plot_dir)) == 0) {
  dirname(args$output)
} else {
  args$plot_dir
}
...
tryCatch(
  {
    ...
    if (is.null(umap_mat) || ncol(umap_mat) < 2) {
      ...
      ggsave(
        file.path(plot_dir, "singler_label_umap.png"),
        p_label,
        width = 11,
        height = 7,
        dpi = 120
      ...
      p_pruned <- ggplot(
        df_plot,
        aes(x = UMAP1, y = UMAP2, color = singler_pruned_label)
      ) +
        geom_point(size = 0.3, alpha = 0.5) +
        base_theme +
      ...
      ggsave(
        file.path(plot_dir, "singler_pruned_label_umap.png"),
        p_pruned,
        width = 11,
        height = 7,
        dpi = 120
      ...
      p_delta <- ggplot(
        df_plot,
        aes(x = UMAP1, y = UMAP2, color = singler_score_delta)
      ) +
        geom_point(size = 0.3, alpha = 0.5) +
      ...
      ggsave(
        file.path(plot_dir, "singler_score_delta_umap.png"),
        p_delta,
        width = 9,
        height = 7,
        dpi = 120
